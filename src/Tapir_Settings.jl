mutable struct Tapir_settings
    ...
    SEQUOIA_Settings(; discretization="Inte_Res", flex_mesh=false, cont_type_x="same_var", cont_type_u="discontinuous", cost_type="path_cost", outer_mesh_start="equidistant", quad_type="gausslegendre", inner_mesh_type="cheb2", interpol="lagrange_bary", resid_tol=10^-6, solver_tol=10^-8, quad_tol=10^-10) = new(discretization, flex_mesh, cont_type_x, cont_type_u, cost_type, outer_mesh_start, quad_type, inner_mesh_type, interpol, resid_tol, solver_tol, quad_tol) #Basic constructor function with defaults
end
