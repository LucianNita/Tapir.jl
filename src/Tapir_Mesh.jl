mutable struct Tapir_mesh
    ...
    evalQX::Matrix{Float64}
    evalQU::Matrix{Float64}
    evalXU::Matrix{Float64}
    evalUX::Matrix{Float64}
...
function set_length!(model)
    ...
    if model.settings.cont_type_x=="same_var"
        model.mesh.st_len_x=model.mesh.st_len_x-model.nx;
    end

    if model.settings.cont_type_u=="same_var"
        model.mesh.st_len_u=model.mesh.st_len_u-model.nu;
    end
end
...
function generate_outer!(mesh, outer_mesh_start)
    if outer_mesh_start=="equidistant"
        mesh.outer_mesh=[-1+2*i/mesh.N for i=0:mesh.N];
    else
...
function generate_interpolation_mesh!(mesh,inner_mesh_type)
    ...
    elseif inner_mesh_type=="equi"
        mesh.inner_x=[-1+2*i/mesh.Px for i=0:mesh.Px];
        mesh.inner_u=[-1+2*i/mesh.Pu for i=0:mesh.Pu];
